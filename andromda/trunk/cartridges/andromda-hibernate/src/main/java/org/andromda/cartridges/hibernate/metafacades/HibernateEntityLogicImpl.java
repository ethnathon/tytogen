 ...
 * remapped to the subclasses. This is useful in the concrete case because it
...
public class HibernateEntityLogicImpl extends HibernateEntityLogic {
	 ...
	 * 
	 * @param entity
	 *            the HibernateEntity from which to retrieve the inheritance
	 *            tagged value.
	...
	protected boolean handleIsHibernateInheritanceUnionSubClass() {
		...
		return (version.startsWith(HibernateGlobals.HIBERNATE_VERSION_3) || version
				.startsWith(HibernateGlobals.HIBERNATE_VERSION_4))
				&& this.getHibernateInheritanceStrategy().equalsIgnoreCase(
						INHERITANCE_STRATEGY_UNION_SUBCLASS);
	}
	...
	protected boolean handleIsLazy() {
		...
		if (StringUtils.isBlank(value)) {
			String version = (String) this
					.getConfiguredProperty(HibernateGlobals.HIBERNATE_VERSION);
			value = version.startsWith(HibernateGlobals.HIBERNATE_VERSION_2) ? "false"
					: "true";
		}
	...
	protected String handleGetFullyQualifiedEntityName() {
		return HibernateMetafacadeUtils.getFullyQualifiedName(
				this.getPackageName(), this.getEntityName(), null);
	}
	...
	protected String handleGetFullyQualifiedEntityImplementationName() {
		return HibernateMetafacadeUtils
				.getFullyQualifiedName(this.getPackageName(),
	...
	protected String handleGetEntityName() {
		String entityNamePattern = (String) this
				.getConfiguredProperty(HibernateGlobals.ENTITY_NAME_PATTERN);

		return MessageFormat.format(entityNamePattern,
	...
	protected String handleGetEntityImplementationName() {
		String implNamePattern = String
				.valueOf(this
						.getConfiguredProperty(HibernateGlobals.ENTITY_IMPLEMENTATION_NAME_PATTERN));

		return MessageFormat.format(implNamePattern,
				StringUtils.trimToEmpty(this.getName()));
	}
	...
	protected String handleGetHibernateDiscriminatorColumn() {
		...
		if (StringUtils.isBlank(column)) {
			column = String
					.valueOf(this
							.getConfiguredProperty(HibernateGlobals.ENTITY_DISCRIMINATOR_COLUMN));
		}
	...
	protected String handleGetHibernateDiscriminatorType() {
		...
		if (StringUtils.isBlank(type)) {
			type = String
					.valueOf(this
							.getConfiguredProperty(HibernateGlobals.ENTITY_DISCRIMINATOR_TYPE));
		}
	...
	public Collection<OperationFacade> getBusinessOperations() {
		return HibernateMetafacadeUtils.filterBusinessOperations(super
	...
	protected boolean handleIsBusinessOperationsPresent() {
		final Collection<OperationFacade> allBusinessOperations = this
	...
	protected int handleGetEhCacheMaxElementsInMemory() {
		String maxElements = (String) this
	...
	protected boolean handleIsHibernateCacheDistributed() {
		String distributed = (String) this
				.getConfiguredProperty(HibernateGlobals.HIBERNATE_ENTITYCACHE_DISTRIBUTED);
		boolean distributedCachingEnabled = Boolean.valueOf(
				StringUtils.trimToEmpty(distributed)).booleanValue();

		if (distributedCachingEnabled) {
			String entityCacheDistributed = (String) this
					.findTaggedValue(HibernateProfile.TAGGEDVALUE_HIBERNATE_ENTITYCACHE_DISTRIBUTED);
			return Boolean.valueOf(
					StringUtils.trimToEmpty(entityCacheDistributed))
					.booleanValue();
		}
		return false;
	}
	...
	protected boolean handleIsTableRequired() {
		return !this.isHibernateInheritanceClass()
				|| (this.isHibernateInheritanceClass() && (this
	...
	protected String handleGetSubclassKeyColumn() {
		...
		if ((superEntity != null)
				&& superEntity.isHibernateInheritanceSubclass()) {
	...
	protected boolean handleIsRequiresMapping() {
		...
		return HibernateUtils
				.mapSubclassesInSeparateFile((String) this
						.getConfiguredProperty(HibernateGlobals.HIBERNATE_MAPPING_STRATEGY))
				|| this.isRoot()
				&& (!this.isHibernateInheritanceInterface()
						|| this.getSpecializations().isEmpty() || (superEntity != null && superEntity
	...
	private boolean isRoot() {
		...
		boolean abstractConcreteEntity = (this.isHibernateInheritanceConcrete() || this
				.isHibernateInheritanceInterface()) && this.isAbstract();
	...
	protected boolean handleIsRequiresSpecializationMapping() {
		return !HibernateUtils
				.mapSubclassesInSeparateFile((String) this
						.getConfiguredProperty(HibernateGlobals.HIBERNATE_MAPPING_STRATEGY))
				&& this.isRoot()
				&& (this.isHibernateInheritanceSubclass()
						|| this.isHibernateInheritanceClass() || this
	...
	protected String handleGetHibernateVersionProperty() {
		String version = (String) this
				.findTaggedValue(HibernateProfile.TAGGEDVALUE_HIBERNATE_VERSION_PROPERTY);
		if (StringUtils.isBlank(version)) {
			version = (String) this
					.getConfiguredProperty(HibernateGlobals.HIBERNATE_VERSION_PROPERTY);
		}
		return version;
	}
	...
	protected String handleGetHibernateVersionPropertySqlName() {
		return EntityMetafacadeUtils
				.toSqlName(
						this.getHibernateVersionProperty(),
	...
	protected String handleGetSequenceName() {
		...
		final int maxLength = this.getMaxSqlNameLength()
				- this.getSequenceSuffix().length();
		if (maxLength > -0) {
			sequenceName = EntityMetafacadeUtils.ensureMaximumNameLength(
					sequenceName, Integer.valueOf(maxLength).shortValue())
					+ sequenceSuffix;
		}
		return sequenceName;
	}
