...
public class JSFActionLogicImpl extends JSFActionLogic {
	...
	private static final Logger LOGGER = Logger
			.getLogger(JSFActionLogicImpl.class);
	...
	/**
	 * Constructs the form bean name, with our without prefixing the use case
	 * name.
	 * 
	 * @param withUseCaseName
	 *            whether or not to prefix the use case name.
	 * @return the constructed form bean name.
	 */
	private String getFormBeanName(boolean withUseCaseName) {
		final String pattern = ObjectUtils.toString(this
				.getConfiguredProperty(JSFGlobals.FORM_BEAN_PATTERN));
		final ModelElementFacade useCase = this.getUseCase();
		final String useCaseName = withUseCaseName && useCase != null ? StringUtilsHelper
				.lowerCamelCaseName(useCase.getName()) : "";
		final String formBeanName = pattern
				.replaceFirst("\\{0\\}", useCaseName);
		final String triggerName = !pattern.equals(formBeanName) ? StringUtils
				.capitalize(this.getTriggerName()) : this.getTriggerName();
	...
	protected String handleGetTriggerName() {
		...
		if (this.isExitingInitialState()) {
			...
			final String suffix = trigger == null ? this.getTarget().getName()
					: trigger.getName();
	...
	protected String handleGetFormImplementationName() {
		final String pattern = ObjectUtils.toString(this
				.getConfiguredProperty(JSFGlobals.FORM_IMPLEMENTATION_PATTERN));
	...
	protected boolean handleIsTableAction() {
		return JSFGlobals.ACTION_TYPE_TABLE.equals(this
				.findTaggedValue(JSFProfile.TAGGEDVALUE_ACTION_TYPE));
	}
	...
	protected String handleGetFullyQualifiedFormImplementationName() {
		...
		return fullyQualifiedName.append(this.getFormImplementationName())
				.toString();
	}
	...
	protected String handleGetFormScope() {
		String scope = ObjectUtils.toString(this
				.findTaggedValue(JSFProfile.TAGGEDVALUE_ACTION_FORM_SCOPE));
		if (StringUtils.isEmpty(scope)) {
			scope = ObjectUtils.toString(this
					.getConfiguredProperty(JSFGlobals.FORM_SCOPE));
		}
		return scope;
	}
	...
	protected String handleGetDocumentationKey() {
		...
		return (event == null ? this.getMessageKey()
				+ ".is.an.action.without.trigger" : event.getMessageKey())
	...
	protected String handleGetDocumentationValue() {
		final String value = StringUtilsHelper
				.toResourceMessage(getDocumentation("", 64, false));
	...
	protected String handleGetTableLinkName() {
		...
		if (value != null) {
			...
			if (tableLink != null) {
				...
				tableLink = columnOffset == -1 ? tableLink : tableLink
						.substring(0, columnOffset);
			}
		}
	...
	protected String handleGetTableLinkColumnName() {
		...
		if (value != null) {
			...
			if (tableLink != null) {
				final int columnOffset = tableLink.indexOf('.');
				tableLink = (columnOffset == -1 || columnOffset == tableLink
						.length() - 1) ? null : tableLink
						.substring(columnOffset + 1);
			}
		}
		return tableLink;
	}
	...
	protected Object handleGetTableLinkParameter() {
		...
		if (tableLinkName != null) {
			...
			if (view != null) {
				...
				for (int ctr = 0; ctr < tables.size()
						&& tableLinkParameter == null; ctr++) {
	...
	protected boolean handleIsHyperlink() {
		...
		return JSFGlobals.ACTION_TYPE_HYPERLINK
	...
	protected List<FrontEndAction> handleGetOtherUseCaseFormActions() {
		final List<FrontEndAction> otherActions = new ArrayList<FrontEndAction>(
				this.getUseCase().getActions());
		for (final Iterator<FrontEndAction> iterator = otherActions.iterator(); iterator
				.hasNext();) {
	...
	protected List<FrontEndParameter> handleGetHiddenParameters() {
		final List<FrontEndParameter> hiddenParameters = new ArrayList<FrontEndParameter>(
				this.getParameters());
		CollectionUtils.filter(hiddenParameters, new Predicate() {
			public boolean evaluate(final Object object) {
				...
				if (object instanceof JSFParameter) {
					...
					if (!valid) {
						for (final Iterator iterator = parameter
								.getAttributes().iterator(); iterator.hasNext();) {
							JSFAttribute attribute = (JSFAttribute) iterator
									.next();
	...
	protected boolean handleIsPopup() {
		boolean popup = ObjectUtils.toString(
				this.findTaggedValue(JSFProfile.TAGGEDVALUE_ACTION_TYPE))
		...
		return Boolean
				.valueOf(
						ObjectUtils.toString(this
								.findTaggedValue(JSFProfile.TAGGEDVALUE_ACTION_FORM_RESET)))
				.booleanValue();
	}
	 ...
	 * 
	 * @param taggedValue
	 *            the tagged value from which to read the message
	 * @return maps message keys to message values, but only those that match
	 *         the arguments will have been recorded
	 */
	private Map<String, String> getMessages(String taggedValue) {
		...
		if (taggedValues.isEmpty()) {
			...
			messages = new LinkedHashMap<String, String>(); // we want to keep
															// the order
			for (final Object tag : taggedValues) {
				final String value = (String) tag;
				messages.put(StringUtilsHelper.toResourceMessageKey(value),
						value);
			}
		}
	...
	protected String handleGetTriggerMethodName() {
		...
		if (this.isExitingInitialState()) {
			final JSFUseCase useCase = (JSFUseCase) this.getUseCase();
			methodName.append(StringUtilsHelper.lowerCamelCaseName(useCase
					.getName()) + "_started");
		} else {
			methodName.append(StringUtilsHelper.lowerCamelCaseName(this
					.getSource().getName()));
			methodName.append('_');
			final EventFacade trigger = this.getTrigger();
			final String suffix = trigger == null ? this.getTarget().getName()
